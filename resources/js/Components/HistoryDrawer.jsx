...
import { X, Search, Clock, Copy, Check, Sparkles, ChevronRight, FileText, Download, CreditCard } from 'lucide-react';
import { toast } from 'sonner';

export default function HistoryDrawer({ isOpen, onClose, history, payments = [], onSelectDrop }) {
  const [activeTab, setActiveTab] = useState('drops'); // 'drops' or 'invoices'
  ...
  const filteredPayments = (payments || []).filter((tx) => {
    const term = searchTerm.toLowerCase();
    return (
      (tx.service_name && tx.service_name.toLowerCase().includes(term)) ||
      (tx.gateway_reference && tx.gateway_reference.toLowerCase().includes(term))
    );
  });
          ...
          <div>
            <div className="flex items-center justify-between pb-4 border-b border-zinc-800">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <h2 className="text-base font-semibold text-white">Account Activity</h2>
                  <p className="text-xs text-zinc-400">Drops history & PDF tax invoices</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            {/* Navigation Tabs */}
            <div className="mt-4 flex rounded-xl bg-zinc-900 p-1 border border-zinc-800">
              <button
                type="button"
              ...
              >
                Product Drops ({history ? history.length : 0})
              </button>
              <button
                type="button"
              ...
              >
                Invoices ({payments ? payments.length : 0})
              </button>
            </div>
            ...
            <div className="mt-3 relative">
              ...
              <input
                type="text"
                placeholder={activeTab === 'drops' ? "Search drops history..." : "Search invoices..."}
                value={searchTerm}
          ...
          <div className="my-4 flex-1 overflow-y-auto pr-1 space-y-3">
            {activeTab === 'drops' ? (
              ...
              filteredPayments.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-48 text-center px-4">
                  <CreditCard className="h-8 w-8 text-zinc-600 mb-2" />
                  <p className="text-sm font-medium text-zinc-400">No invoice records found</p>
                  <p className="text-xs text-zinc-600 mt-1">Invoices will appear here automatically upon credit purchases or drops.</p>
                </div>
              ) : (
                filteredPayments.map((tx) => (
                  <div
                    key={tx.id}
                    className="rounded-xl border border-zinc-800/80 bg-zinc-900/50 p-4 transition-all duration-200 hover:border-zinc-700"
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="flex items-center space-x-2">
                          <span className={`inline-block h-2 w-2 rounded-full ${tx.type === 'topup' ? 'bg-emerald-400' : 'bg-purple-400'}`} />
                          <p className="text-xs font-bold text-white">{tx.service_name}</p>
                        </div>
                        <p className="text-[11px] font-mono text-zinc-400 mt-1">Ref: {tx.gateway_reference}</p>
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-xs font-black text-white">€{parseFloat(tx.amount).toFixed(2)}</p>
                        <a
                          href={route('wallet.invoice', tx.id)}
                          target="_blank"
                          rel="noreferrer"
                          className="mt-2 inline-flex items-center space-x-1 rounded-lg border border-purple-500/30 bg-purple-500/10 px-2.5 py-1 text-[11px] font-bold text-purple-300 hover:bg-purple-600 hover:text-white transition"
                        >
